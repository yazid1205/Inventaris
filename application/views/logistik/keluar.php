...
<div class="card">
	<div class="card-header">
		<div class="row">
			<div class="col-md-6">
				<h6 class="mt-1 font-weight-semibold">Data Barang Keluar</h6>
			</div>
			<div class="col-md-6 text-right">
				...
				<a href="<?php echo base_url('logistik/addkeluar') ?>" class="btn btn-primary btn-sm shadow-sm"><i class="fe fe-plus-circle mr-1"></i>Tambah</a>
				<?php endif ?>
			</div>
		</div>
	</div>
	<div class="card-body">
		...
		<div class="table-responsive">
			<table class="table table-striped table-bordered dataTable">
				...
				<tbody>
					<?php foreach ($keluar->result() as $x => $d): ?>
					<tr>
						<td><?=$x+1?></td>
						<td><?=$d->nm_logistik?></td>
						<td><?=$d->jml?></td>
						...
						<?php if ($akun->level == 0): ?>
						<td>
							<a href="<?=base_url('logistik/deletedata/keluar/' . $d->id)?>" onclick="return confirm('Yakin ingin menghapus data ?')" class="btn btn-danger btn-sm shadow-sm"><i class="fe fe-x"></i></a>
							<button class="btn btn-success btn-sm shadow-sm" data-toggle="modal" data-target="#updatekeluar_<?=$d->id?>"><i class="fe fe-edit-2"></i></button>
						</td>
						<?php endif ?>
					</tr>
					<?php endforeach ?>
...
<?php if ($akun->level == 0): ?>
<?php foreach ($keluar->result() as $d): ?>
<div class="modal fade" id="updatekeluar_<?=$d->id?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  	<div class="modal-dialog" role="document">
	    <div class="modal-content">
	        ...
	        <form action="<?=base_url('logistik/updatekeluar/' . $d->id)?>" method="post">
