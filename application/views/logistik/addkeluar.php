<div class="card">
	<div class="card-body">
		<div class="row">
			<div class="col-md-6">
				<h5 class="mt-1 font-weight-semibold">Tambah Barang Keluar</h5>
			</div>
			<div class="col-md-6 text-right">
				<a href="<?=base_url('logistik/keluar')?>" class="btn btn-warning btn-sm shadow-sm"><i class="fe fe-arrow-left mr-1"></i>Kembali</a>
			</div>
		</div>
		<hr>
					...
					<div class="form-group">
						...
						<select name="logistik_id" class="form-control" required="">
							...
							<?php foreach ($logistik->result() as $e): ?>
								<option value="<?=$e->id?>"><?=$e->nm_logistik?> - <?=$e->jml?></option>
							<?php endforeach ?>
						</select>
					</div>
				</div>
